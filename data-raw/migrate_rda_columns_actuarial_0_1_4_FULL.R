...
# -------------------------------------------------------------------------
# Script único para migrar columnas de data/*.rda a notación actuarial compacta.
#
# Uso:
#   1. Copia este archivo en:
#        data-raw/migrate_rda_columns_actuarial_0_1_4.R
#   2. Desde la raíz del paquete, ejecuta:
...
#        devtools::check()
#
# El script crea copia de seguridad en:
...
migrate_mortality_colombia_tables <- function() {
  migrate_object(
    object_name = "mortality_colombia_tables",
    mapping = c(
      table = "table_id",
      age = "x",
      mu = "mu_x",
      qx_calculated = "qx_calc",
      qx_difference = "qx_diff"
    ),
    required = c(
      "table_id",
      "sex",
...
run_rda_column_migration <- function() {
  ...
  migrate_mortality_colombia_tables()
