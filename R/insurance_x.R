...
#' )
#'
#' # Whole life insurance: A_60 = 1 - d * ä_60
...
insurance_x <- function(
  ...
  # --- whole life: A_x = 1 - d * ä_x (Finan, Sec. 37.1) ---
  if (type == "whole") {
    adue_y <- annuity_x(
      lt = lt, x = y, i = i,
      n = NULL, m = 0L, k = 1L,
    ...
    if (type == "endowment") {
      # A_{x:n} = 1 - d * ä_{x:n} (Finan, Sec. 26.3.2 / Example 37.8)
      result <- defer * (1 - d * adue_y_n)
    } else {
      # A^1_{x:n} = 1 - d * ä_{x:n} - v^n * n_p_y (Finan, Sec. 27)
      # = A_{x:n} - nEx
      n_p_y <- t_p_int(y, n)
