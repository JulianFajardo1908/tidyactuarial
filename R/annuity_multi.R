...
#' Supports cohort annuities (first-death / last-survivor) and a joint-and-survivor
#' style annuity ("reversionary") that pays 1 while all lives are alive and then
#' pays a fraction alpha while at least one life remains alive.
#'
#' @param lt Life table with column \code{x} and at least one of \code{lx}, \code{px}, \code{qx}.
#' @param ages Integer vector of actuarial ages for the lives at issue.
#' @param annuity Type of annuity. \code{"cohort"} uses \code{cohort}. \code{"reversionary"}
#'   uses \code{alpha}.
#' @param cohort \code{"first"} (pays while all alive) or \code{"last"} (pays while at least one alive).
#'   Used only when \code{annuity = "cohort"}.
#' @param alpha Reversionary fraction (typically in \eqn{[0,1]}). Used only when
#'   \code{annuity = "reversionary"}. \code{alpha = 0} matches first-death; \code{alpha = 1}
#'   matches last-survivor.
#' @param n Integer term in years after deferment. If \code{NULL}, runs to the end of the table.
#' @param m Integer deferment in years.
#' @param k Integer payments per year. If \code{k > 1}, Woolhouse may be used.
...
#'
#' @return A single numeric value: the APV.
#'
#' @examples
#' lt <- data.frame(x = 60:110, lx = seq(100000, 0, length.out = 51))
#'
#' # Joint-life annuity-due for 3 lives
#' annuity_multi(lt, ages = c(60, 62, 65), i = 0.05,
#'               annuity = "cohort", cohort = "first", timing = "due")
#'
#' # Last-survivor annuity-due for 3 lives
#' annuity_multi(lt, ages = c(60, 62, 65), i = 0.05,
#'               annuity = "cohort", cohort = "last", timing = "due")
#'
#' @export
annuity_multi <- function(
  ...
  if (woolhouse == "first") {
    ...
    # p_status1 ≈ status_prob(m+1) / status_prob(m)
