...
#'   \item Valuation is performed at coupon dates (no accrued interest).
#' }
#'
...
#'   from issue. Each value must lie between \code{0} and \code{years_to_maturity} and
#'   must align with coupon dates.
...
#'
#' @return Numeric vector of book values, one for each \code{valuation_time}.
#'
#' @details
...
#'
#' @seealso \code{\link{bond_book_value_tbl}}, \code{\link{bond_price}}, \code{\link{bond_cash_flows}},
...
#' Book value table of a level coupon bond at coupon dates
#'
#' Computes the book value of a level coupon bond at one or more coupon dates
#' and returns the results in tibble form.
#'
#' This is a reporting wrapper around the book-value calculation. The book
#' value is interpreted prospectively and is evaluated immediately after any
#' coupon due at the valuation time has been paid.
#'
#' Assumptions:
#' \itemize{
#'   \item Coupons are paid in arrears at regular intervals.
...
#' @param coupon_rate Numeric scalar. Annual coupon rate as a proportion.
#' @param years_to_maturity Numeric scalar. Final maturity in years.
#' @param valuation_time Numeric vector. Valuation time(s) in years.
#' @param coupons_per_year Positive integer. Number of coupon payments per year.
#' @param y_effective_per_period Optional numeric scalar. Effective yield per
#'   coupon period. If supplied, it is used directly.
#' @param y_rate Optional numeric scalar. Annual yield rate value.
...
#' \describe{
#'   \item{valuation_time}{Valuation time in years.}
#'   \item{valuation_period}{Valuation period index on the coupon grid.}
#'   \item{book_value}{Prospective book value at the valuation time.}
#'   \item{yield_per_period}{Effective yield per coupon period used in valuation.}
...
#'   \item{redemption}{Final redemption value.}
#' }
#'
#' @seealso \code{\link{bond_book_value}}, \code{\link{bond_price}}, \code{\link{bond_cash_flows}},
#'   \code{\link{bond_duration}}, \code{\link{bond_convexity}}
#'
#' @family bonds
#'
...
.bond_book_value_schedule <- function(
  ...
  if (isTRUE(check)) {
    ...
    if (!is.numeric(valuation_time) || any(is.na(valuation_time)) ||
...
# (These should live in a bond-utils.R file in the package)
# ============================================================

#' Validate common bond scalar inputs
#' @noRd
.validate_bond_core <- function(face, coupon_rate, years_to_maturity,
                                coupons_per_year, y_m, redemption, tol) {
  ...
  if (!is.numeric(y_m) || length(y_m) != 1L || is.na(y_m) ||
      !is.finite(y_m) || y_m <= 0 ||
...
#' @noRd
.resolve_bond_yield <- function(y_effective_per_period = NULL,
                                y_rate = NULL,
                                y_type = "effective",
                                y_m = 1L,
                                coupons_per_year = 1L) {
  if (!is.null(y_effective_per_period)) {
    if (!is.numeric(y_effective_per_period) || length(y_effective_per_period) != 1L ||
        is.na(y_effective_per_period) || !is.finite(y_effective_per_period) ||
    ...
    if (is.null(y_rate)) {
      stop(
        "Provide either `y_effective_per_period` or `y_rate`.",
        call. = FALSE
      )
    }
